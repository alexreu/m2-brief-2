...
def drop_most_incomplete_rows(df: pd.DataFrame, pct: float = 0.02) -> pd.DataFrame:
    row_missing = get_row_missing_percentage(df)
    number_rows_to_drop = int(len(df) * pct)
...
# Pour les colonnes avec un taux de valeurs manquantes élevé (>= 25%)
# on utilise KNNImputer pour une imputation plus intelligente.
def impute_missing_values(df: pd.DataFrame) -> pd.DataFrame:
    numeric_columns = df.select_dtypes(include="number").columns.tolist()
    high_missing_columns = [
        column for column in numeric_columns
        # On cible les colonnes avec un taux de valeurs manquantes élevé pour l'imputation KNN.
        if df[column].isna().mean() * 100 >= 25.0
    ]
    ...
    if high_missing_columns:
        ...
        for column in high_missing_columns:
            df[column] = imputed_numeric[column]

    for column in df.columns:
        # La mediane reste utile sur les colonnes peu incompletes.
